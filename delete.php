	 ...
	 if(isset($_GET['delete_id']))
	 {
         ...
         while($row=$stmt->fetch(PDO::FETCH_BOTH))
         {
			 ...
			 <td>
							...
							while($xrow=$data->fetch(PDO::FETCH_ASSOC)){
									
									if ($row['idpais'] == $xrow['codigo'])
									{
									   echo '<option value="'.$xrow['codigo'].'" selected>'.$xrow['nombre'].'</option>'; // POSICIONA EN LA FILA 
									}
									else
									{
									   echo '<option value="'.$xrow['codigo'].'">'.$xrow['nombre'].'</option>';
									}   
							}
							?>							
